...
describe AppSent::ConfigFile do
  ...
  context ".new" do
    ...
    %w(valid? constantized).each do |method|
    ...
    context "should send right variables to AppSent::ConfigValue" do
      ...
      it "in a simple case" do
        ...
        AppSent::Settings.new(:path => fixtures_path, :env => 'test', :caller => __FILE__) do
          database do
            username String
          end
        end
  ...
  context "#valid?" do
    ...
    def yaml_loads_file_and_return(*something)
      YAML.should_receive(:load_file).once.with(fake_config_filename).and_return(*something)
    end

    context "should be true" do

      it "if config exists and environment presence(without type)(no values)" do
        yaml_loads_file_and_return('environment' => {:a=>100500})
        subject.new(*params).should be_valid
      end

      it "if config exists and environment presence(with type specified)(no values)" do
        @params['type']=Array
        yaml_loads_file_and_return(:environment => [1,2,3])
        subject.new(*params).should be_valid
      end
      ...
      it "if config exists and environment presence(with values)" do
        ...
        yaml_loads_file_and_return('environment' => {:value=>'100500'})
    ...
    context "should be false" do
      ...
      it "if environment does not presence in config" do
        yaml_loads_file_and_return('other_environment' => {:a=>100500})
      end

      it "if wrong type" do
        @params['type'] = Array
        yaml_loads_file_and_return(:environment => 123)
      end
